...
class DepthWise_Res(Module):
    ...
    def forward(self, x):
        ...
        x = self.conv_bn_prelu(x)
        x = self.conv_bn(x)
        # reference :[https://arxiv.org/abs/1801.04381.pdf]: intro MultiDepthWise_Res on mobilenetV2
        # khong can downsample(x), vi so luong chanel kha nho
        # MultiDepthWise_Res: giai thich tai [https://jarvislabs.ai/blogs/resnet]
        output = short_cut + x
        return output
...
class MobileFaceNet(Module):
    def __init__(self, embedding_size):
        ...
        self.conv1 = Conv_Bn_Prelu(3, 64, kernel=3, stride=2, padding=1)  # 1
        self.conv2 = Conv_Bn_Prelu(64, 64, kernel=3, stride=1, padding=1, groups=64)  # 2
        self.conv3 = DepthWise(64, 64, kernel=3, stride=2, padding=1, groups=128)  # 3
        self.conv4 = MultiDepthWise_Res(64, kernel=3, stride=1, padding=1, num_block=4, groups=128)  # 4
        self.conv5 = DepthWise(64, 128, kernel=3, stride=2, padding=1, groups=256)  # 5
        self.conv6 = MultiDepthWise_Res(128,  kernel=3, stride=1, padding=1,num_block=6, groups=256)  # 6
        self.conv7 = DepthWise(128, 128, kernel=3, stride=2, padding=1, groups=512)  # 7
        self.conv8 = MultiDepthWise_Res(128,  kernel=3, stride=1, padding=1,num_block=2, groups=256)  # 8
        self.conv9 = Conv_Bn_Prelu(128, 512, kernel=1, stride=1, padding=0)  # 9
        self.conv10 = Conv_Bn(512, 512,  kernel=7, stride=1, padding=0,groups=512)  # 10
