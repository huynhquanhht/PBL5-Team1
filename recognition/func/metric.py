...
class Arcface(Module):
    def __init__(self, embedding_size=512, classnum=5124,  s=64., m=0.5):
        # class_num: so luong nguoi muon train
    ...
    def forward(self, embbedings, label):
        ...
        cos_theta = cos_theta.clamp(-1, 1)  # Dua cos ve khoang [-1,1]
        cos_theta_pow_2 = torch.pow(cos_theta, 2)
        sin_theta_pow_2 = 1 - cos_theta_pow_2
        sin_theta = torch.sqrt(sin_theta_pow_2)
        ...
        output *= self.s  # scale de lam softmax hoat dong, tham khao trong normface
